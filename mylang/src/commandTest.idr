...
factorial =
    Seq
        (NatAssign (N) (NatLitExpr nat_two)) --CHANGE N TO WHATEVER NATURAL NUMBER
        (Seq
            (NatAssign (Y) (NatLitExpr nat_one))
            (While
                (Nnot0)
                (Seq
                    (NatAssign (Y) (NatMultExpr (NatVarExpr Y) (NatVarExpr N)))
                    (NatAssign (N) (NatMinusExpr (NatVarExpr N) (NatLitExpr nat_one))))))

-- RUN THIS IN THE REPL TO GET FACTORIAL: (getNatState (CommandEval factorial state_init)) Y

----------------------------------------------------------------------------------------
-- BELOW IS A FUNCTION THAT TAKES ANY NATURAL NUMBER AND RETURNS FACTORIAL WHEN EXECUTING COMMAND --
----------------------------------------------------------------------------------------

factorialFunc: Nat -> Command
factorialFunc n =
    Seq
        (NatAssign (N) (NatLitExpr n))
        (Seq
            (NatAssign (Y) (NatLitExpr nat_one))
            (While
                (Nnot0)
...
-- RUN THE FOLLOWING IN REPL TO FIND FACTORIAL:
-- (getNatState (CommandEval (factorialFunc [NATURAL NUMBER]) state_init)) Y
