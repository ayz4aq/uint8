...
reduces to a) "v _ _ _ _" Hint: the answer is one word, five letters
...
{-
2 [5 pts]. Evaluating a command in a given state yields a "s _ _ _ _"
-}
...
e1: NatExpr
e1 = NatPlusExpr (NatVarExpr X) (NatLitExpr nat_one)
...
n1: Nat
n1 = natExprEval e1 state_init
-- expect nat_one -> S Z


-- The follow code defines st as a shorthand for init_state.
...
c1: Command
c1 = NatAssign (X) (NatLitExpr nat_five)

{-
5b. What value is assigned to X by init_state (st)? <NatLitExpr nat_five>
-}
...
c2: Command
c2 = NatAssign (X) (NatPlusExpr (NatVarExpr X) (NatLitExpr nat_one))
...
When evaluating c3 in a given state, st, where c3 is a sequential composition of
two smaller functions (c1 and c2), the code evaluates c2 in the state generated by
evaluating c1 in the initial state (st). As a result, evaluating the command will
result in a state that will appear as if the commands are executed sequentially.
...
When evaluating a While command with a condition, c, and a body, b, in a state,
st, the following occurs. The program will first evaluate c in st, and if c is True,
then the program will then evaluate another While command in the state generated
by evaluating the command on the state passed to the function. This is done recursively
until c evaluates to False, which then the function then returns the state, which
should represent the desired value.
...
Option is necessary because in the case that a value does not exist at a certain
key, then Option can hold a 'None' value. In the expected case that there is a value
at a given key, then Option will hold that value.
...
data Foo = MkFoo (Nat) (Bool) (List Nat)
